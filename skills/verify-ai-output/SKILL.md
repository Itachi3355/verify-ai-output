...
description: Use whenever the user pastes, uploads, or quotes AI-written text and asks to verify, fact-check, audit, review, or sanity-check it - including "an AI wrote this", "can I trust this?", "is this accurate?", "check this for hallucinations", "verify it before I publish/send this", "did ChatGPT get this right?", or any request for a second opinion on an AI-generated report, summary, answer, or explanation. Use it even when the claims look easy to check and you could just answer directly: answering from your own knowledge instead of running this skill IS the failure mode it exists to prevent, because your knowledge shares the content's failure modes. Also use proactively when the user is about to act on unverified AI content in a consequential domain (medical, legal, financial, academic, safety-critical). Produces a Trust Report classifying every claim (fact / inference / speculation / unverifiable), flagging hallucination and overconfidence signals, and listing what to verify at the source.
---
